...
const ExamModalStepFourComponent = ({ setStep, selectedRows, loading, data, handleClose, setComments, comments, submitData }) => {
    const { user } = usePage().props.auth;

    const handleDownloadAdmitCard = () => {
        // For now, create a simple admit card download
        // In a real application, this would call a backend endpoint
        const admitCardData = {
            studentName: data.name,
            studentId: data.unique_id,
            department: data.department,
            exams: selectedRows.map((row) => row.exam),
            date: new Date().toLocaleDateString(),
        };

        // Create a simple text-based admit card
        const admitCardContent = `
==============================================
              ADMIT CARD
         মাদরাসাতুল হেরা টাঙ্গাইল
==============================================

Student Name: ${admitCardData.studentName}
Student ID: ${admitCardData.studentId}
Department: ${admitCardData.department}
Date: ${admitCardData.date}

Exams:
${admitCardData.exams.map((exam, index) => `${index + 1}. ${exam}`).join('\n')}

==============================================
        `;

        // Create downloadable file
        const blob = new Blob([admitCardContent], { type: 'text/plain' });
        const url = window.URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `admit_card_${data.unique_id}_${new Date().toISOString().split('T')[0]}.txt`;
    ...
    return (
        <div className="space-y-3">
            ...
            <div className="flex items-center justify-between rounded-lg bg-[#F2F2F2] p-3">
                ...
                <button onClick={handleDownloadAdmitCard} className="flex items-center gap-2 rounded px-3 py-2 transition-colors hover:bg-[#E6E6E6]">
